...
The model identifies an hyperplane which is able to separate leavers from non-leavers with the lowest error. In a multidimensional space with the decision variables as dimensions (activity log and demographic information), key variables are combined to form the best decision boundary.
Out of all 41 variables available, the project identifies 20 variables, without performance deterioration, able to predict which users are starting to lose interest in the app. This allows to focus on a limited number of key variables, meaning that more than half of the variables have no predictive performance.
